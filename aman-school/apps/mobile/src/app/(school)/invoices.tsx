...
/* ---- BC-6 / invoices: official invoice list + refund request (school) ---- */
export default function SchoolInvoicesScreen() {
  ...
  const { data: invoices } = useQuery({ queryKey: ["school-invoices", schoolId], queryFn: () => api.school.invoices(schoolId) as Promise<any[]> });

  const refundMutation = useMutation({
    mutationFn: () => {
      const lastPaid = invoices?.find((i) => i.status === "paid");
      if (!lastPaid) throw new Error("لا توجد فاتورة مدفوعة لطلب استرداد بشأنها");
      return api.school.requestRefund({
        subjectType: "school", subjectId: schoolId, reason,
...
const styles = StyleSheet.create({
  ...
  amount: { fontWeight: "700", color: colors.amber, fontSize: 13 },
