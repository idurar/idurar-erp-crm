...
const LeadModel = mongoose.model('Lead'); // Assuming you have a Lead model defined
const crudController = require('../corsControllers/crudController');
const methods = crudController.createCRUDController('Lead');
  ...
  try {
    ...
    const result = await LeadModel.aggregate([
      ...
      },
      {
        $project: {
          _id: 0,
          new: {
            $multiply: [{ $divide: ['$new', totalLeads] }, 100],
          },
          active: {
            $multiply: [{ $divide: ['$active', totalLeads] }, 100],
          },
        },
      },
    ]);
    ...
    if (!result || !result.length) {
      return res.status(200).json({
        ...
        message: 'Successfully get summary of new leads',
      });
    }
