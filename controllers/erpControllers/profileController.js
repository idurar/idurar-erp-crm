...
const methods = crudController.createCRUDController('Profile');
...
methods.summary = async (req, res) => {
  try {
    ...
    const totalProfiles = await Model.countDocuments({
      removed: false,
      created: {
    ...
    const result = await Model.aggregate([
      ...
      },
      {
        $project: {
          _id: 0,
          new: {
            $multiply: [{ $divide: ['$new', totalProfiles] }, 100],
          },
          active: {
            $multiply: [{ $divide: ['$active', totalProfiles] }, 100],
          },
        },
      },
    ]);
    ...
    if (!result || !result.length) {
      return res.status(200).json({
        ...
        message: 'Successfully get summary of new profiles',
      });
    }
